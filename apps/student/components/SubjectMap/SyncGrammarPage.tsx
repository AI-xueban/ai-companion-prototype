...
import type { EnglishGrammarSection, EnglishGrammarTopic, EnglishGrammarUnit } from '../../data/juniorEnglishSyncGrammarG7A';
...
interface SyncGrammarPageProps {
  open: boolean;
  units: EnglishGrammarUnit[];
  onBack: () => void;
  onPlayTopic: (unit: EnglishGrammarUnit, section: EnglishGrammarSection, topic: EnglishGrammarTopic) => void;
}

export const SyncGrammarPage: React.FC<SyncGrammarPageProps> = ({
  open,
  units,
  onBack,
  onPlayTopic,
        ...
        >
          <header className="relative shrink-0 border-b border-slate-100 bg-white px-5 pb-3 pt-4">
            <div className="flex items-center gap-2.5">
              ...
              <h1 className="min-w-0 flex-1 truncate text-[18px] font-semibold text-slate-900">语法精讲</h1>
            </div>
          </header>
                    ...
                    <div key={section.id} className="space-y-3">
                      {section.title !== '语法精讲' && (
                        <div className="flex items-center gap-2">
                          <span className="h-4 w-[3px] rounded-full bg-indigo-400" />
                          <span className="text-[14px] font-semibold text-slate-700">{section.title}</span>
                        </div>
                      )}
                      {section.topics.length > 0 && (
                        <div className="grid grid-cols-2 gap-3">
                          {section.topics.map((topic) => {
                            const hash = hashText(`${unit.id}-${section.id}-${topic.id}`);
                            ...
                            return (
                              <button
                                key={topic.id}
                                type="button"
                                onClick={() => onPlayTopic(unit, section, topic)}
                              ...
                              >
                                ...
                                <div className="px-3 py-2.5">
                                  <p className="line-clamp-2 text-[13px] font-semibold leading-5 text-slate-800">{topic.title}</p>
                                </div>
                              </button>
                            );
                          })}
